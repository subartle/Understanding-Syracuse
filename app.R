...
Dat.DistTimeDiff <-
  merge(DrivingDat.Tract_Dist,
        Dat.Tract_Dist,
        by.x = "NAME",
        by.y = "NAME")

Dat.DistTimeDiff <- Dat.DistTimeDiff[, c(1:5, 8:10, 17:19)]
colnames(Dat.DistTimeDiff) <-
  c(
    "Census Tract",
    "Concat",
    "lon",
...
Dat.DistTimeDiff$Difference <-
  Dat.DistTimeDiff$MinutesperMile.Bus - Dat.DistTimeDiff$MinutesperMile.Car
...
Dat.DistTimeDiffTable <- Dat.DistTimeDiff[, c(1, 6:12)]
Dat.DistTimeDiffTable <- 
  Dat.DistTimeDiffTable[order(Dat.DistTimeDiff$Difference, decreasing = TRUE), ]
      ...
      tabPanel(
        ...
        tabsetPanel(
          ...
          tabPanel(
            ...
            h4("Description:"),
            h5(
              "Time and distance estimates taken from Google Maps (https://www.google.com/maps)"
            ),
            h5(
            ...
            h4(
              div(
                "What is the time difference between a PERSONAL VEHICLE and PUBLIC TRANSPORTATION?",
                style = "color:red"
              )
            ),
            fixedRow(
              column(12, leafletOutput("TransportationMap5", height = "700px"))),
            fixedRow(column(
              3,
              numericInput("TransportObs3", "# of rows:", 17),
              h5(
                "The above map looks at the difference in the minutes per mile using a personal vehicle versus using
                public transportation (from the center of each census tract to downtown)."
              ),
              h4("Observations:")
              ),
              column(9,
                     tableOutput("TransportTable3")))
            ))
    ), 
    ...
    server <- function(input, output, session) {
      ...
      observeEvent(
        ...
        ),
        {
          ...
          output$TransportationMap5 <- renderLeaflet({
            leaflet(Dat.TransDifference) %>%
              ...
              addMarkers(
                ...
                ~ lat,
                icon = nhoodIcon,
                popup = paste("Census Tract: ", Dat.TransDifference$NAME)
              ) %>%
              addPolygons(
                stroke = FALSE,
                fillOpacity = 0.7,
                smoothFactor = 0.5,
                color = ~ colorNumeric("Reds", Dat.TransDifference$Difference)(Dat.TransDifference$Difference),
                popup = paste(
                  "Difference in minutes per mile (bus - car): ",
                  Dat.TransDifference$Difference
                )
              )
          })
          ...
          output$TransportTable3 <- renderTable({
            head(Dat.DistTimeDiffTable, n = input$TransportObs3)
          })
